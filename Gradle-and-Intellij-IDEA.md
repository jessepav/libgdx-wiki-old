...
## Running Your Project
  ...
  * **iOS**: Before you can run on iOS you need to [install the RoboVM IntelliJ IDEA plugin or Intel Multi-OS Engine](http://robovm.mobidevelop.com). `Run -> Edit Configurations...`, click the plus (+) button and select `RoboVM iOS`. Set the `Name` to `iOS`. The module should be set to iso if not already done. 
